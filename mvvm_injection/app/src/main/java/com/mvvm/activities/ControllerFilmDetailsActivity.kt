        ...
        if (!movie_id.isNullOrEmpty()) {
            viewModel.dettaglioFilm(movie_id)?.observe(this, { it ->
                ...
                viewModel.getActorsList(movie_id)?.observe(this, { it ->
                    ...
                    Log.e(TAG , "size listGenres               = ${resultDetailsFilm.listGenres!!.size}")
                    Log.e(TAG , "size listProductionCompanies  = ${resultDetailsFilm.listProductionCompanies!!.size}")
                    Log.e(TAG , "size listProductionCountries  = ${resultDetailsFilm.listProductionCountries!!.size}")
                    Log.e(TAG , "size listSpokenLanguages      = ${resultDetailsFilm.listSpokenLanguages!!.size}")
                    Log.e(TAG , "size listCast                 = ${resultActorList.listCast!!.size}")
                    Log.e(TAG , "size listCrew                 = ${resultActorList.listCrew!!.size}")
                    ...
                    filmDetailsBinding.filmTitolo.text =
                        FilmApplicationGlobal.fixTooLongTitle(resultDetailsFilm.title)

    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (filmApplicationGlobal.resultDetailsFilm != null && filmApplicationGlobal.resultActorList != null) {
            ...
            filmDetailsBinding.actorsRecyclerView.adapter =
                AdapterActor(filmApplicationGlobal.resultActorList!!.listCast!!)

            val url =
                FilmApplicationGlobal.GET_PICTURE_URL + filmApplicationGlobal.resultDetailsFilm!!.backdropPath
            Glide.with(this).load(url).into(filmDetailsBinding.filmDetailImagine)
            filmDetailsBinding.filmTitolo.text =
                FilmApplicationGlobal.fixTooLongTitle(filmApplicationGlobal.resultDetailsFilm!!.title.toString())
            filmDetailsBinding.filmDataValue.text =
                filmApplicationGlobal.resultDetailsFilm!!.releaseDate.toString()
            filmDetailsBinding.filmImdbIdValue.text =
                filmApplicationGlobal.resultDetailsFilm!!.imdbId.toString()
            filmDetailsBinding.filmLinkHomePageValue.text =
                filmApplicationGlobal.resultDetailsFilm!!.homepage.toString()
            ...
            if (filmApplicationGlobal.resultDetailsFilm?.listProductionCompanies.isNullOrEmpty() || filmApplicationGlobal.resultDetailsFilm?.listProductionCountries.isNullOrEmpty()) {
                ...
                filmDetailsBinding.filmCountryValue.text =
                    filmApplicationGlobal.resultDetailsFilm?.listProductionCompanies!![0].originCountry
                filmDetailsBinding.filmFromValue.text =
                    filmApplicationGlobal.resultDetailsFilm?.listProductionCompanies!![0].name
            }
            if (filmApplicationGlobal.resultDetailsFilm?.listSpokenLanguages.isNullOrEmpty()) {
                ...
                filmDetailsBinding.filmLinguaValue.text =
                    filmApplicationGlobal.resultDetailsFilm?.listSpokenLanguages!![0].englishName
            }
            if (filmApplicationGlobal.resultDetailsFilm?.overview.isNullOrEmpty()) {
                filmDetailsBinding.filmOverviewValue.text = ""
            } else {
                filmDetailsBinding.filmOverviewValue.text =
                    filmApplicationGlobal.resultDetailsFilm?.overview
            }
        }
    }
}
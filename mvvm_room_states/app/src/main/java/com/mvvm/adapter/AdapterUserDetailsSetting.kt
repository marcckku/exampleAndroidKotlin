...
class AdapterUserDetailsSetting(private val users: List<User>) :
    ...
    inner class UserViewHolder(val userProfileRowBinding: UserProfileRowBinding) :
        ...
        fun bind(user: User) {
            with(itemView) {
                ...
                userProfileRowBinding.userProfile.setOnClickListener {
                    val intent = Intent(
                        userProfileRowBinding.root.context,
                        UserDetailsSettingActivity::class.java
                    )
                    intent.putExtra("user.id", user.id)
                    userProfileRowBinding.root.context.startActivity(intent)
                }
            }
        }
    }
